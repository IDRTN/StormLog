    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.summaryGrid}>
          {summary.maxWind != null && (
            <View style={styles.summaryItem}>
              ...
              <Text style={[styles.summaryValue, { color: Colors.wind }]}>
                {Math.round(summary.maxWind)} mph
              </Text>
            </View>
          )}
          {summary.maxGust != null && (
            <View style={styles.summaryItem}>
              ...
              <Text style={[styles.summaryValue, { color: Colors.warning }]}>
                {Math.round(summary.maxGust)} mph
              </Text>
            </View>
          )}
          {summary.avgHumidity != null && (
            <View style={styles.summaryItem}>
              ...
              <Text style={[styles.summaryValue, { color: Colors.humidity }]}>
                {summary.avgHumidity}%
              </Text>
            </View>
          )}
        ...
        {summary.minPressure != null && summary.maxPressure != null && (
          <View style={styles.pressureRow}>
            ...
            <Text style={[styles.summaryValue, { color: Colors.pressure }]}>
              {summary.maxPressure.toFixed(2)} → {summary.minPressure.toFixed(2)}"
            </Text>
          </View>
        )}
        ...
        {summary.totalPrecip != null && summary.totalPrecip > 0 && (
          <View style={styles.pressureRow}>
            ...
            <Text style={[styles.summaryValue, { color: Colors.precipitation }]}>
              {summary.totalPrecip.toFixed(2)}"
            </Text>
          </View>
        )}
        ...
        {summary.alertCount > 0 && (
          <View style={styles.alertSection}>
            ...
            {summary.alertTypes.length > 0 && (
              <Text style={styles.alertTypes}>{summary.alertTypes.join(', ')}</Text>
            )}
          </View>
        )}
      ...
      {records.length >= 2 && (
        <>
          ...
          {temps.length >= 2 && (
            <SimpleGraph title="Temperature (°F)" values={temps} color={Colors.temperature} unit="°F" />
          )}
          {pressures.length >= 2 && (
            <SimpleGraph title="Pressure (inHg)" values={pressures} color={Colors.pressure} unit='"' />
          )}
          {humidities.length >= 2 && (
            <SimpleGraph title="Humidity (%)" values={humidities} color={Colors.humidity} unit="%" />
          )}
          {winds.length >= 2 && (
            <SimpleGraph title="Wind Speed (mph)" values={winds} color={Colors.wind} unit=" mph" />
          )}
        </>
      )}
        ...
        <>
          ...
          {records.map((obs) => (
            <View key={obs.id} style={styles.obsCard}>
              ...
              {obs.nwsAlerts && (
                <View style={styles.obsAlertRow}>
                  <Ionicons name="warning" size={10} color={Colors.warning} />
                  <Text style={styles.obsAlertText}>{obs.nwsAlerts}</Text>
                </View>
              )}
            </View>
          ))}
...
const styles = StyleSheet.create({
  ...
  alertTypes: { color: Colors.warning, fontSize: 11 },
  ...
  obsAlertRow: { flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: SPACING.xs },
  obsAlertText: { color: Colors.warning, fontSize: 10 },
});
